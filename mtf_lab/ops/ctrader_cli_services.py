...
def _persist_discovery(
    config: Any, *, token_ref: str, observed: Mapping[str, Any], observed_at: datetime | None = None
) -> Path:
...
def _ctrader_config(config: Any) -> Any:
    ...
    values.setdefault(
        "client_id", os.environ.get(str(oauth.get("client_id_env", ""))) if oauth.get("client_id_env") else None
    )
    values.setdefault(
        "client_secret_ref", str(oauth.get("client_secret_env", "")) if oauth.get("client_secret_env") else None
    )
...
def _read_callback_input(args: argparse.Namespace) -> str:
    ...
    if use_stdin:
        ...
        path = Path(cast(str | os.PathLike[str], callback_file)).expanduser()
        flags = os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0)
        try:
            fd = os.open(path, flags)
...
class CTraderCliService:
    ...
    def doctor(self, args: argparse.Namespace) -> CommandResult:
        ...
        if getattr(args, "network", False):
            ...
            try:
                output["connectivity"] = _check_ctrader_connectivity(
                    config, timeout=float(getattr(args, "timeout", 5.0))
                )
            except Exception as exc:
                output["connectivity"] = {
                    "requested": True,
        ...
        output["connector_usable"] = bool(
            report.available
            and output.get("activation", {}).get("status", {}).get("ready", False)
    ...
    @staticmethod
    def _activation_status(config: Any, status_command: Callable[..., dict[str, Any]]) -> Mapping[str, Any]:
        ...
        return {
            ...
            "ready": False,
            "next_action": "Use config/ctrader_query.toml o registre la aplicación; no se abrió navegador.",
        }
    ...
    def auth_url(self, args: argparse.Namespace) -> CommandResult:
        ...
        if not config.ctrader_oauth or not config.ctrader:
            return _json_error(
                {
                    "ok": False,
                    "state": "APP_CREDENTIALS_REQUIRED",
                    "next_action": "Seleccione un perfil cTrader completo.",
                    "browser_opened": False,
                }
            )
            ...
            return _json_error(
                {
                    ...
                    "network_performed": False,
                    "browser_opened": False,
                    "next_action": "Habilite explícitamente el perfil antes de iniciar OAuth.",
                }
            )
        requested_scope = (
            str(getattr(args, "scope", "") or ("trading" if profile.operation_mode.value == "DEMO" else "accounts"))
            .strip()
            .lower()
        )
            ...
            return _json_error(
                {
                    ...
                    "network_performed": False,
                    "browser_opened": False,
                    "next_action": "Solicite sólo un scope permitido por el perfil y autorícelo en una fase separada.",
                }
            )
            ...
            return _json_error(
                {
                    "ok": False,
                    "state": "APP_CREDENTIALS_REQUIRED",
                    "missing": [app.client_id_env],
                    "next_action": "Defina client_id localmente; nunca lo guarde en TOML.",
                    "browser_opened": False,
                }
            )
    ...
    def callback_listen(self, args: argparse.Namespace) -> CommandResult:
        ...
        if not getattr(args, "attempt_id", None):
            return _json_error(
                {
                    "ok": False,
                    "state": "CALLBACK_REQUIRED",
                    "network_performed": False,
                    "next_action": "Proporcione --attempt-id devuelto por auth-url.",
                }
            )
        app = OAuthAppConfig.from_mapping(dict(config.ctrader_oauth))
        profile = ActivationProfile.from_mapping(dict(config.ctrader))
        ...
        return CommandResult.json(
            {
                ...
                "attempt": attempt.to_public_dict(),
                "network_performed": False,
                "browser_opened": bool(getattr(args, "open_browser", False)),
                "next_action": "Ejecute token-exchange --attempt-id para intercambiar el código persistido; no se imprimió el código.",
            }
        )
    ...
    def _fixture_exchange(args: argparse.Namespace) -> CommandResult:
        ...
        with tempfile.TemporaryDirectory(prefix="mtf-lab-ctrader-fixture-") as tmp:
            ...
            output = {
                ...
                "fixture_source": "local_oauth_fixture",
    ...
    def _real_exchange(args: argparse.Namespace) -> CommandResult:
            ...
            return _json_error(
                {
                    ...
                    "state": "CALLBACK_REQUIRED",
                    "network_performed": False,
                    "next_action": "Use auth-url y entregue --attempt-id más --callback-file 0600 o --callback-stdin; no pase el código como argumento.",
                }
            )
        if getattr(args, "callback_uri", None):
            return _json_error(
                {
                    "ok": False,
                    "state": "CALLBACK_INPUT_UNSAFE",
                    "network_performed": False,
                    "next_action": "No pase el callback con código en argumentos; use --callback-file 0600 o --callback-stdin.",
                }
            )
        client_id = os.environ.get(app.client_id_env, "")
            ...
            return _json_error(
                {
                    ...
                    "state": "APP_CREDENTIALS_REQUIRED",
                    "network_performed": False,
                    "next_action": "Defina las variables locales de aplicación; no se guardaron secretos.",
                }
            )
        network_performed = False
        try:
    ...
    def _selection_data(self, args: argparse.Namespace, config: Any) -> tuple[Any, Any, list[Any]] | CommandResult:
        ...
        if not accounts_path.exists():
            return _json_error(
                {
                    "ok": False,
                    "state": "ACCOUNT_DISCOVERY_REQUIRED",
                    "next_action": "Ejecute query --network o proporcione --accounts-file con una respuesta observada; no se selecciona automáticamente.",
                }
            )
        ...
        if metadata is None:
            return _json_error(
                {
                    "ok": False,
                    "state": "TOKEN_REFERENCE_REQUIRED",
                    "network_performed": False,
                    "next_action": "Active primero un token externo con scope accounts.",
                }
            )
            ...
            return _json_error(
                {
                    ...
                    "network_performed": False,
                    "next_action": "El archivo debe contener una lista accounts observada.",
                }
            )
            ...
            return _json_error(
                {
                    ...
                    "state": "ACCOUNT_DISCOVERY_INVALID",
                    "network_performed": False,
                    "next_action": "El token_ref del descubrimiento no coincide con el perfil.",
                }
            )
    ...
    def _make_discovery(metadata: Any, raw: Any, accounts: list[Any]) -> Any | CommandResult:
        ...
        try:
            return account_discovery_command(
                metadata,
                accounts,
                observed_at=CTraderCliService._parse_observed_at(raw),
                permission_scope=(
                    raw.get("permissionScope", raw.get("permission_scope")) if isinstance(raw, Mapping) else None
                ),
            )
        except Exception as exc:
            return _json_error(
    ...
    def select(self, args: argparse.Namespace) -> CommandResult:
        ...
        output = select_account_command(
            _ctrader_activation_payload(config), discovery, account_id=args.account_id, environment="DEMO"
        )
        try:
            selection_path = _persist_selection(
                config, account_id=args.account_id, token_ref=metadata.token_ref, observed_at=discovery.observed_at
            )
        except Exception as exc:
            return _json_error(
                {
        ...
        output["next_action"] = (
            "Selección DEMO guardada fuera del repo; al conectar se volverá a validar contra el inventario del servidor."
        )
        return CommandResult.json(output)
        ...
        return _json_error(
            {
                ...
                "network_performed": False,
                "activation": status,
                "next_action": "Use --fixture o --network sólo tras completar OAuth externo.",
            }
        )
            ...
            return _json_error(
                {
                    ...
                    "state": "APP_CREDENTIALS_REQUIRED",
                    "network_performed": False,
                    "next_action": "Defina las referencias de aplicación en variables de entorno.",
                }
            )
    ...
    def _query_observation(self, context: QueryContext, provider: Any, observed: Any) -> CommandResult:
        ...
        status_after = status_command(
            ...
            token_metadata=context.metadata,
            present_env_keys=set(os.environ),
            accounts=accounts,
            now=datetime.now(UTC),
        )
        ...
        provider.authorize_account(
            int(context.profile.account_id),
            token_provider=lambda ref: context.lease.access_token if ref == context.profile.token_ref else "",
        )
    ...
    @staticmethod
    def _secret_provider(app: Any, client_secret: str, sequence: list[str]) -> Callable[[str], str]:
    ...
    @staticmethod
    def _token_provider(profile: Any, lease: Any, sequence: list[str]) -> Callable[[str], str]:
        def provide(ref: str) -> str:
            if ref == profile.token_ref:
                sequence.append("account_discovery")
                return str(lease.access_token)
            return ""
    ...
    def _try_persist_discovery(config: Any, profile: Any, observed: Any) -> Path | None:
        try:
            return _persist_discovery(
                config, token_ref=profile.token_ref, observed=observed, observed_at=datetime.now(UTC)
            )
        except Exception:
            return None
            ...
            return CommandResult.json(
                {
                    ...
                    "fixture_source": "local_fixture",
                    "gateway_adapter_used": False,
                    "next_action": "Use --activate sólo para el fixture local determinista; no se activa una cuenta externa ni se contacta gateway.",
                }
            )
        return self.fixture(args)
