...
def normalize_pair(pair: str) -> str:
    ...
    internal_aliases = {
        "XXBTZUSD": "BTC/USD", "XBTZUSD": "BTC/USD", "XXBT/USD": "BTC/USD",
        "XXBTZUSDT": "BTC/USDT", "XBTZUSDT": "BTC/USDT",
    }
...
class KrakenPublicAdapter:
    ...
    def fetch_ohlc(
        ...
        *,
        ...
        if interval not in _SUPPORTED_INTERVALS:
            raise KrakenConfigurationError(f"unsupported Kraken OHLC interval: {interval}; allowed={sorted(_SUPPORTED_INTERVALS)}")
        ...
        request_uri = f"{self.rest_endpoint}?{query}"
        ...
        for row_index, row in enumerate(rows):
            if not isinstance(row, (list, tuple)) or len(row) < 8:
                raise KrakenAPIError(f"Kraken OHLC row {row_index} has {len(row) if isinstance(row, (list, tuple)) else 'non-array'} fields; expected >=8")
            try:
        ...
        if "last" in result:
            try:
                last_timestamp = datetime.fromtimestamp(int(result["last"]), tz=UTC)
            except (TypeError, ValueError, OverflowError) as exc:
                raise KrakenAPIError(f"invalid Kraken result.last: {result['last']!r}") from exc
        elif bars:
            last_timestamp = bars[-1].interval_start
    ...
    @staticmethod
    def _raise_body_error(payload: Mapping[str, Any], *, request_uri: str, raw_hash: str, http_status: int | None = None) -> None:
        ...
        try:
            async for message in messages:
                ...
                for candle in data:
                    ...
                    bar = self._normalize_ohlc_update(candle, interval=interval, observed_at=observed_at, revision=revisions)
                    if message_type == "snapshot":
                        self.status.snapshot_seen = True
                    emitted += 1
                    yield bar
                ...
                async with websockets.connect(
                    ...
                    while True:
                        ...
                        if message.get("channel") != channel:
                            # pong/heartbeat/subscription acknowledgements are
                            # still observed for status but are not data rows.
                            continue
    ...
    def _normalize_trade(self, trade: Mapping[str, Any], *, is_snapshot: bool) -> Event:
        ...
        source_id = str(trade_id) if trade_id is not None else hashlib.sha256(json.dumps(dict(trade), sort_keys=True, default=str).encode()).hexdigest()
