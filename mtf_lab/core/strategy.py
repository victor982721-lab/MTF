...
from .models import Candle, OperationMode, Timeframe, parse_timeframe
...
class StrategyConfig:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "preparation_lookback", _as_positive_int(self.preparation_lookback, "preparation_lookback"))
        object.__setattr__(self, "preparation_ttl_bars", _as_positive_int(self.preparation_ttl_bars, "preparation_ttl_bars"))
        object.__setattr__(self, "max_distance_atr", _as_number(self.max_distance_atr, "max_distance_atr", minimum=0.0))
        object.__setattr__(self, "rsi_threshold", _as_number(self.rsi_threshold, "rsi_threshold", minimum=0.0, maximum=100.0))
    ...
    def from_mapping(cls, mapping: Mapping[str, Any] | None) -> "StrategyConfig":
        ...
        _strict_keys(raw, allowed, "strategy")
        values = dict(raw)
        # Alias de la configuración de ejemplo del proyecto. Se normalizan a
        # los nombres semánticos del núcleo y se rechazan colisiones explícitas.
        ...
        if timeframes is not None:
            if isinstance(timeframes, Mapping):
                _strict_keys(dict(timeframes), {"context", "preparation", "trigger", "context_timeframe", "preparation_timeframe", "trigger_timeframe"}, "strategy.timeframes")
            ...
            elif isinstance(timeframes, Sequence) and not isinstance(timeframes, (str, bytes)):
                parsed = [parse_timeframe(value) for value in timeframes]
                by_seconds = {tf.seconds: tf for tf in parsed}
                for target, default_seconds in (("trigger_timeframe", 60), ("preparation_timeframe", 300), ("context_timeframe", 900)):
        ...
        if ttl_minutes is not None:
            # El TTL se expresa en unidades de la temporalidad de preparación
            # para mantener una semántica determinista incluso si cambia M5.
            try:
            ...
            preparation_tf = parse_timeframe(values.get("preparation_timeframe", cls().preparation_timeframe))
            ...
            ttl_bars = int(ttl_seconds) // preparation_tf.seconds
            if "preparation_ttl_bars" in values:
                raise ValueError("preparation_ttl_minutes no puede combinarse con preparation_ttl_bars")
            values["preparation_ttl_bars"] = ttl_bars
        for grouping_name, fields in (
            ("lookbacks", {"context": "context_lookback", "preparation": "preparation_lookback", "context_lookback": "context_lookback", "preparation_lookback": "preparation_lookback"}),
            ("conditions", {"max_distance_atr": "max_distance_atr", "rsi_threshold": "rsi_threshold", "preparation_ttl_bars": "preparation_ttl_bars"}),
        ):
        ...
        if indicators is not None:
            values["indicators"] = IndicatorConfig.from_mapping(indicators)
        return cls(**values)
...
def _quality_from(value: Any) -> DataQuality:
    ...
    if isinstance(value, Mapping):
        ...
        for flag in flags:
            try:
                parsed.add(QualityFlag(str(flag).lower()))
            except ValueError:
                parsed.add(QualityFlag.INVALID)
        if value.get("status") and str(value["status"]).lower() not in {"valid", "synthetic"}:
            try:
                parsed.add(QualityFlag(str(value["status"]).lower()))
            except ValueError:
                parsed.add(QualityFlag.INVALID)
        return DataQuality(frozenset(parsed), tuple(str(item) for item in value.get("reasons", ())))
...
class TrendPullbackStrategy:
    ...
    def _prepare_stream(self, value: Any, timeframe: Timeframe) -> _PreparedStream:
        ...
        if value is None:
            return _PreparedStream(timeframe, "unknown", (), DataQuality.from_flag(QualityFlag.INSUFFICIENT, "stream_missing"))
        if isinstance(value, Mapping):
    ...
    def _context_snapshot(self, stream: _PreparedStream, index: int) -> _ContextSnapshot:
        ...
        if not self._point_quality_ok(lag, stream.quality) or lag.ema_slow is None or point.ema_fast is None or point.ema_slow is None:
            required_reason = "context_indicator_not_ready"
            return _ContextSnapshot(point, None, False, {**values, "ema_slow_lag": getattr(lag, "ema_slow", None), "reason": required_reason})
    ...
    def _decision(self, conditions: Sequence[ConditionResult], *, preparation: bool = False, duplicate: bool = False) -> DecisionKind:
        mandatory = [condition for condition in conditions if condition.mandatory]
        if duplicate or any(condition.reason in {"preparation_expired", "context_invalidated", "signal_duplicate"} for condition in mandatory):
    ...
    def _preparation_evaluation(
        ...
        if context is None:
            conditions.append(self._condition("context_direction", ConditionState.UNKNOWN, None, "UP/DOWN", "context_not_available"))
        elif context.direction in {"UP", "DOWN"}:
            conditions.append(self._condition("context_direction", ConditionState.FULFILLED, context.direction, "UP/DOWN"))
        elif context.known:
            conditions.append(self._condition("context_direction", ConditionState.FAILED, context.direction, "UP/DOWN", "context_neutral"))
        else:
            conditions.append(self._condition("context_direction", ConditionState.UNKNOWN, None, "UP/DOWN", "context_indicator_not_ready"))
        ...
        if not self._point_quality_ok(point, stream.quality) or point.close is None:
            conditions.append(self._condition("pullback", ConditionState.UNKNOWN, point.close, "directional_close_change", "preparation_quality_blocked"))
            conditions.append(self._condition("distance_to_ema", ConditionState.UNKNOWN, None, f"<= {self.config.max_distance_atr} ATR", "preparation_quality_blocked"))
        elif lag is None or not self._point_quality_ok(lag, stream.quality) or lag.close is None:
            conditions.append(self._condition("pullback", ConditionState.UNKNOWN, None, "directional_close_change", "preparation_lookback_insufficient"))
            conditions.append(self._condition("distance_to_ema", ConditionState.UNKNOWN, None, f"<= {self.config.max_distance_atr} ATR", "preparation_lookback_insufficient"))
        else:
            ...
            if direction == "UP":
                ...
                pullback = False
            conditions.append(self._condition("pullback", ConditionState.FULFILLED if pullback else ConditionState.FAILED, point.close - lag.close, "< 0 (UP) or > 0 (DOWN)", "" if pullback else "directional_pullback_failed"))
            if point.ema_fast is None or point.atr is None:
                conditions.append(self._condition("distance_to_ema", ConditionState.UNKNOWN, None, f"<= {self.config.max_distance_atr} ATR", "ema_or_atr_not_ready"))
            else:
                ...
                conditions.append(self._condition("distance_to_ema", ConditionState.FULFILLED if within else ConditionState.FAILED, distance, f"<= {limit}", "" if within else "distance_exceeded"))
        quality_condition = ConditionState.FULFILLED if self._point_quality_ok(point, stream.quality) else ConditionState.UNKNOWN
        conditions.append(self._condition("data_quality", quality_condition, point.quality.status, "valid and closed", "" if quality_condition is ConditionState.FULFILLED else "data_quality_blocked"))
        conditions.append(self._condition("news_filter", ConditionState.DISABLED, None, "not configured", "not_configured", mandatory=False))
    ...
    def _trigger_evaluation(
        ...
        if active is not None and available >= active.expires_at:
            invalid_reason = "preparation_expired"
            active = replace(active, invalidated=True, invalidation_reason=invalid_reason)
            active_for_eval = None
        elif active is not None and active.invalidated:
            invalid_reason = active.invalidation_reason or "context_invalidated"
            active_for_eval = None
        else:
            active_for_eval = active
        direction = active_for_eval.direction if active_for_eval else (context.direction if context and context.direction in {"UP", "DOWN"} else None)
        expected_context = direction or "UP/DOWN"
        conditions: list[ConditionResult] = []
        if context is None:
            conditions.append(self._condition("context_revalidated", ConditionState.UNKNOWN, None, expected_context, "context_not_available"))
        elif direction in {"UP", "DOWN"} and context.direction == direction:
            conditions.append(self._condition("context_revalidated", ConditionState.FULFILLED, context.direction, direction))
        elif not context.known:
            conditions.append(self._condition("context_revalidated", ConditionState.UNKNOWN, context.direction, expected_context, "context_indicator_not_ready"))
        else:
            conditions.append(self._condition("context_revalidated", ConditionState.FAILED, context.direction, direction, "context_invalidated"))
            invalid_reason = invalid_reason or "context_invalidated"
        if active_for_eval is None:
            ...
            conditions.append(self._condition("preparation_episode", state, None, "registered and unexpired", reason))
        elif active_for_eval.used:
            conditions.append(self._condition("preparation_episode", ConditionState.FAILED, active_for_eval.episode_id, "unused episode", "signal_duplicate"))
        else:
            conditions.append(self._condition("preparation_episode", ConditionState.FULFILLED, active_for_eval.episode_id, "registered and unexpired"))
        previous = stream.points[index - 1] if index > 0 else None
        if direction not in {"UP", "DOWN"}:
            conditions.append(self._condition("close_cross_ema", ConditionState.UNKNOWN, None, "cross in direction", "direction_not_known"))
        elif not self._point_quality_ok(point, stream.quality) or point.close is None or point.ema_fast is None:
            conditions.append(self._condition("close_cross_ema", ConditionState.UNKNOWN, None, "cross in direction", "trigger_indicator_not_ready"))
        elif previous is None or not self._point_quality_ok(previous, stream.quality) or previous.close is None or previous.ema_fast is None or previous.end != point.start:
            conditions.append(self._condition("close_cross_ema", ConditionState.UNKNOWN, None, "cross in direction", "trigger_previous_bar_unavailable"))
        else:
            if direction == "UP":
                crossed = previous.close <= previous.ema_fast and point.close > point.ema_fast
            else:
                crossed = previous.close >= previous.ema_fast and point.close < point.ema_fast
            conditions.append(self._condition("close_cross_ema", ConditionState.FULFILLED if crossed else ConditionState.FAILED, {"previous_close": previous.close, "close": point.close}, direction, "" if crossed else "no_directional_cross"))
        if direction not in {"UP", "DOWN"} or point.rsi is None or not self._point_quality_ok(point, stream.quality):
            conditions.append(self._condition("rsi_threshold", ConditionState.UNKNOWN, point.rsi, self.config.rsi_threshold, "rsi_not_ready"))
        else:
            passed = point.rsi > self.config.rsi_threshold if direction == "UP" else point.rsi < self.config.rsi_threshold
            conditions.append(self._condition("rsi_threshold", ConditionState.FULFILLED if passed else ConditionState.FAILED, point.rsi, f"> {self.config.rsi_threshold}" if direction == "UP" else f"< {self.config.rsi_threshold}", "" if passed else "rsi_threshold_failed"))
        quality_condition = ConditionState.FULFILLED if self._point_quality_ok(point, stream.quality) else ConditionState.UNKNOWN
        conditions.append(self._condition("data_quality", quality_condition, point.quality.status, "valid and closed", "" if quality_condition is ConditionState.FULFILLED else "data_quality_blocked"))
        conditions.append(self._condition("news_filter", ConditionState.DISABLED, None, "not configured", "not_configured", mandatory=False))
        ...
        if decision is DecisionKind.SIGNAL and active_for_eval is not None and context is not None and direction is not None:
            ...
            next_active = replace(active_for_eval, used=True)
        evaluation = Evaluation(
            timestamp=point.end,
            available_at=available,
            instrument=instrument,
    ...
    def evaluate(self, streams: Mapping[Any, Any]) -> StrategyResult:
        ...
        config = self.config
        context_stream = self._prepare_stream(_stream_value(streams, parse_timeframe(config.context_timeframe)), parse_timeframe(config.context_timeframe))
        preparation_stream = self._prepare_stream(_stream_value(streams, parse_timeframe(config.preparation_timeframe)), parse_timeframe(config.preparation_timeframe))
        trigger_stream = self._prepare_stream(_stream_value(streams, parse_timeframe(config.trigger_timeframe)), parse_timeframe(config.trigger_timeframe))
        instruments = {stream.instrument for stream in (context_stream, preparation_stream, trigger_stream) if stream.instrument not in {"", "unknown"}}
        ...
        # Eventos auxiliares de contexto/preparación se procesan antes del
        # disparador si comparten disponibilidad; el rango de prioridad define
        # el orden simultáneo de forma explícita.
        ...
        triggers: list[tuple[datetime | None, int, IndicatorPoint]] = []
        for index, point in enumerate(trigger_stream.points):
            triggers.append((_point_available(point), index, point))
        ...
        for trigger_available, trigger_index, trigger_point in triggers:
            # Una vela M1 abierta no tiene available_at y se vuelve una
            # evaluación bloqueada, sin forzar el avance de datos auxiliares.
            evaluation_time = trigger_point.end
            watermark = trigger_available
            if watermark is not None:
                while auxiliary_index < len(auxiliary) and auxiliary[auxiliary_index][0] <= watermark:
                    _available, _priority, item_index, kind = auxiliary[auxiliary_index]
                    if kind == "context":
                        context = self._context_snapshot(context_stream, item_index)
                        if active is not None and (context.direction != active.direction or not context.known):
                            invalidation_reason = "context_invalidated"
                            active = replace(active, invalidated=True, invalidation_reason=invalidation_reason)
                            active = None
                    else:
                        prep_evaluation, episode = self._preparation_evaluation(preparation_stream, item_index, context, instrument)
                        ...
                        if episode is not None:
                            ...
                            invalidation_reason = ""
                    auxiliary_index += 1
            # Si la señal está a una hora posterior al último dato de contexto,
            # la última snapshot conocida se revalida pero no se inventa otra.
            if trigger_available is None:
                conditions = (
                    self._condition("data_quality", ConditionState.UNKNOWN, trigger_point.quality.status, "valid and closed", "trigger_not_available"),
            ...
            if invalidation_reason and active is None and not any(condition.reason == invalidation_reason for condition in ()):  # keep explicit below
                ...
                # el episodio anterior todavía es relevante.
                pass
            trigger_evaluation, signal, next_active = self._trigger_evaluation(trigger_stream, trigger_index, context, active, instrument)
            if invalidation_reason and "context_invalidated" not in trigger_evaluation.reasons and active is None:
                # No se altera la decisión ni se transforma unknown en failed;
                # se conserva evidencia de la invalidación en reasons.
                trigger_evaluation = replace(trigger_evaluation, reasons=tuple(dict.fromkeys((*trigger_evaluation.reasons, invalidation_reason))))
