...
class CTraderProviderTests(unittest.TestCase):
    def test_config_catalog_and_secret_redaction(self) -> None:
        ...
        configured = CTraderConfig(
            client_id="public-id",
            client_secret_ref="vault://ctrader/client",
            access_token_ref="vault://ctrader/token",
            account_id=7,
        )
    ...
    def test_relative_trendbar_scaling_and_interval(self) -> None:
        ...
        bar = normalize_trendbar(
            synthetic_trendbar(timestamp_minutes=100),
            spec=spec,
    ...
    def test_optional_quote_fields_do_not_become_zero_or_another_basis(self) -> None:
        ...
        result = normalize_spot_event(
            {"symbolId": 99, "timestamp": int(BASE.timestamp() * 1000), "bid": 110000},
            spec=spec,
        ...
        bid_result = normalize_spot_event(
            {"symbolId": 99, "timestamp": int(BASE.timestamp() * 1000), "bid": 110000},
            spec=spec,
            received_at=BASE + timedelta(seconds=1),
            quote_basis="bid",
    ...
    def test_provider_retains_partial_bid_ask_legs_and_their_ages(self) -> None:
        ...
        self.assertEqual(
            event.metadata["ask_source_timestamp"], (BASE + timedelta(seconds=0)).isoformat().replace("+00:00", "Z")
        )
    ...
    def test_spot_event_can_contain_multiple_trendbars(self) -> None:
        ...
        payload = synthetic_spot_event(
            ...
            symbol_id=99,
            trendbars=[
                synthetic_trendbar(timestamp_minutes=1, period="M1"),
                synthetic_trendbar(timestamp_minutes=5, period="M5"),
            ],
        )
        result = normalize_spot_event(payload, spec=spec, received_at=BASE + timedelta(seconds=1))
        ...
        def handler(request: WireMessage):
            return [
                ...
                WireMessage(
                    ...
                    {"symbol": [{"symbolId": 99, "symbolName": "EUR/USD"}]},
    ...
    def test_protocol_error_reads_body_and_retry_after(self) -> None:
        def handler(request: WireMessage):
            return WireMessage(
                "PROTO_OA_ERROR_RES",
                {"errorCode": "BLOCKED_PAYLOAD_TYPE", "description": "rate limited", "retryAfter": 2},
                request.client_msg_id,
            )
    ...
    def test_reconnect_backoff_is_injectable_and_deterministic(self) -> None:
        ...
        client = CTraderClient(
            CTraderConfig(
                symbol_id=99, max_reconnects=3, reconnect_backoff_seconds=0.25, reconnect_backoff_max_seconds=1
            ),
            transport=transport,
            sleep=delays.append,
        )
            ...
            if request.payload_type_id == PAYLOAD["PROTO_OA_GET_TRENDBARS_REQ"]:
                return WireMessage(
                    "PROTO_OA_GET_TRENDBARS_RES",
                    {
                        ...
                        "trendbar": [
                            synthetic_trendbar(timestamp_minutes=100),
                            synthetic_trendbar(timestamp_minutes=101),
                        ],
                        "hasMore": False,
                    },
                    request.client_msg_id,
    ...
    def test_provider_catalog_and_subscription_order(self) -> None:
        def handler(request: WireMessage):
            if request.payload_type_id == PAYLOAD["PROTO_OA_SYMBOLS_LIST_REQ"]:
                return WireMessage(
                    "PROTO_OA_SYMBOLS_LIST_RES",
                    {"symbol": [{"symbolId": 42, "symbolName": "EUR/USD", "digits": 5, "pipPosition": 4}]},
                    request.client_msg_id,
                )
            response_name = {
                PAYLOAD["PROTO_OA_SUBSCRIBE_SPOTS_REQ"]: "PROTO_OA_SUBSCRIBE_SPOTS_RES",
                PAYLOAD["PROTO_OA_SUBSCRIBE_LIVE_TRENDBAR_REQ"]: "PROTO_OA_SUBSCRIBE_LIVE_TRENDBAR_RES",
    ...
    def test_oauth_request_sequence_is_official_and_values_are_redacted_from_wire_view(self) -> None:
        def handler(request: WireMessage):
            responses = {
                PAYLOAD["PROTO_OA_APPLICATION_AUTH_REQ"]: WireMessage(
                    "PROTO_OA_APPLICATION_AUTH_RES", {}, request.client_msg_id
                ),
                PAYLOAD["PROTO_OA_GET_ACCOUNTS_BY_ACCESS_TOKEN_REQ"]: WireMessage(
                    "PROTO_OA_GET_ACCOUNTS_BY_ACCESS_TOKEN_RES",
                    {"ctidTraderAccount": [{"ctidTraderAccountId": 7, "isLive": False}]},
                    request.client_msg_id,
                ),
                PAYLOAD["PROTO_OA_ACCOUNT_AUTH_REQ"]: WireMessage(
                    "PROTO_OA_ACCOUNT_AUTH_RES", {"ctidTraderAccountId": 7}, request.client_msg_id
                ),
            }
            return responses[request.payload_type_id]

        config = CTraderConfig(
            client_id="public", client_secret_ref="secret-ref", access_token_ref="token-ref", account_id=7, symbol_id=99
        )
        transport = DeterministicTransport(handler)
        client = CTraderClient(config, transport=transport)
        client.connect()
        self.assertEqual(
            client.authenticate(secret_provider=lambda ref: "SECRET_VALUE", token_provider=lambda ref: "TOKEN_VALUE"),
    ...
    def test_account_auth_rejects_server_observed_live_account(self) -> None:
        def handler(request: WireMessage):
            ...
            if request.payload_type_id == PAYLOAD["PROTO_OA_GET_ACCOUNTS_BY_ACCESS_TOKEN_REQ"]:
                return WireMessage(
                    "PROTO_OA_GET_ACCOUNTS_BY_ACCESS_TOKEN_RES",
                    {"ctidTraderAccount": [{"ctidTraderAccountId": 7, "isLive": True}]},
                    request.client_msg_id,
                )
            raise AssertionError(request.payload_type_name)
    ...
    def test_application_oauth_discovers_accounts_without_account_id_or_auto_selection(self) -> None:
        ...
        self.assertEqual(
            discovery["records"][1], {"account_id": 8, "environment": "UNKNOWN", "permission_scope": "SCOPE_VIEW"}
        )
        self.assertEqual(
    ...
    def test_real_generated_account_payload_does_not_expose_token_and_requires_full_symbol_entity(self) -> None:
        ...
        self.assertEqual(
            normalized["records"], [{"account_id": 7, "environment": "DEMO", "permission_scope": "SCOPE_TRADE"}]
        )
...
class CTraderCalendarTests(unittest.TestCase):
    def test_calendar_does_not_infer_open_or_quote_without_explicit_window(self) -> None:
        ...
        calendar = CTraderMarketCalendar(
            [CTraderSessionWindow(weekday=3, open_minute=0, close_minute=1440)], holidays=["2026-01-01"]
        )
...
class CTraderTransportHardeningTests(unittest.TestCase):
    ...
    def test_official_codec_envelope_and_heartbeat_when_extra_is_available(self) -> None:
        ...
        request = WireMessage(
            "PROTO_OA_SYMBOLS_LIST_REQ", {"ctidTraderAccountId": 7, "includeArchivedSymbols": False}, "req-1"
        )
    ...
    def test_rate_limits_are_separate_for_historical_and_other_requests(self) -> None:
        ...
        limiter.acquire(historical=False)
        limiter.acquire(historical=False)
        limiter.acquire(historical=False)
        limiter.acquire(historical=True)
        limiter.acquire(historical=True)
...
class CTraderReconnectTests(unittest.TestCase):
    ...
    def test_periodic_heartbeat_is_driven_by_poll_without_reactor(self) -> None:
        ...
        transport = DeterministicTransport()
        client = CTraderClient(
            CTraderConfig(symbol_id=99, heartbeat_seconds=10), transport=transport, clock=lambda: now[0]
        )
        client.connect()
        now[0] = 11.0
    ...
    def test_tcp_transport_receive_pump_correlates_responses_and_events(self) -> None:
        ...
        class FakeTcp:
            ...
            def receive(self, timeout=None):
                try:
                    return self.inbound.get(timeout=timeout or 0.05)
                except queue.Empty:
                    return None
        ...
        self.assertEqual(response.payload_type_name, "PROTO_OA_SYMBOLS_LIST_RES")
        self.assertIsNotNone(client.poll_event(0.2))
        client.close()
