...
def fixture_event(seconds: int, price: float, event_id: str) -> Event:
    ...
    return Event(
        instrument="TEST/USD", event_time=timestamp, price=price, quantity=1.0,
        source="fixture", source_event_id=event_id,
        received_at=timestamp, available_at=timestamp,
    )
...
class RuntimeOperationalTests(unittest.TestCase):
    def test_open_capture_keeps_missing_final_pending_until_complete(self) -> None:
        ...
        processor.register_signal(Signal(
            signal_id="sig", instrument="TEST/USD", direction="UP", detected_at=BASE,
            context_start=BASE, preparation_start=BASE, trigger_start=BASE,
            trigger_end=BASE + timedelta(minutes=1), episode_id="episode",
        ))
    ...
    def test_checkpoint_restores_operational_gate_and_observation_book(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            with SQLiteStore(Path(tmp) / "runtime.sqlite3") as store:
                session_id = store.create_session(mode="LIVE", provider="fixture", instrument=live_config.instrument, config=live_config.to_dict())
                coordinator = RuntimeCoordinator(store, session_id, live_config, mode="LIVE", dataset_hash="fixture", clock=clock)
                coordinator.update_feed_state(
                    connection="DISCONNECTED", reconciliation="NEEDS_RECONCILIATION", freshness="STALE",
                    blocked_reasons=("feed_stale",), block_details={"feed_stale": {"source": "fixture"}},
                )
                coordinator.checkpoint()
                resumed = RuntimeCoordinator(store, session_id, live_config, mode="LIVE", dataset_hash="fixture", clock=clock)
