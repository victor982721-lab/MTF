...
from mtf_lab.ops.backtest import BacktestRunner
...
def point(ts: datetime, price: float, ordinal: int, *, available_at: datetime | None = None) -> dict:
    return {
        ...
        "price_base": "close",
        "closed": True,
        "available_at": (available_at or ts).isoformat().replace("+00:00", "Z"),
...
class OpsTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with SQLiteStore(db) as store:
                ...
                store.save_checkpoint(
                    sid, "replay", cursor={"row": 2}, events_processed=2, last_event_id="e2", state={"warm": False}
                )
    ...
    def test_backtest_partition_excludes_crossing_exploration(self) -> None:
        start = datetime(2025, 1, 1, tzinfo=UTC)
        signals = [
            {"signal_id": "a", "detected_ts": (start + timedelta(seconds=60)).isoformat(), "direction": "UP"},
            {"signal_id": "b", "detected_ts": (start + timedelta(seconds=120)).isoformat(), "direction": "UP"},
        ]
        points = [point(start + timedelta(seconds=x), 100 + x / 60, x) for x in range(0, 600, 60)]
        runner = BacktestRunner(
            spec=EvaluationSpec(horizons_seconds=(60,), entry_latency_seconds=1, max_price_age_seconds=120)
        )
    ...
    def test_demo_is_offline_and_generates_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with SQLiteStore(result["db_path"], read_only=True) as store:
                ...
                self.assertIn("SINTETICO", Path(result["report_path"]).read_text(encoding="utf-8") or "")
