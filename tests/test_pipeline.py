...
from mtf_lab.core import DataQuality, MarketEvent, OperationMode, aggregate_events, assess_freshness
...
from mtf_lab.pipeline import run_synthetic_pipeline
...
class PipelineAcceptanceTests(unittest.TestCase):
    ...
    def test_virtual_outcomes_and_break_even(self) -> None:
        ...
        def p(sec: int, value: float) -> dict:
            return {
                "timestamp": (start + timedelta(seconds=sec)).isoformat(),
                "price": value,
                "price_base": "close",
                "closed": True,
            }
    ...
    def test_feed_and_candle_ages_are_separate(self) -> None:
        ...
        fresh = assess_freshness(
            ...
            last_closed_end=now - timedelta(minutes=10),
            max_feed_age_seconds=90,
            max_closed_candle_age_seconds=900,
        )
        stale = assess_freshness(
            now=now,
            last_received_at=now - timedelta(seconds=100),
            last_closed_end=now - timedelta(minutes=10),
            max_feed_age_seconds=90,
            max_closed_candle_age_seconds=900,
        )
